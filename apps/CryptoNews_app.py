...
def app():
    ...
    search_option = st.sidebar.radio(
        "🔎 Crypto News Finder", options=["Top Headlines", "Search Keyword"]
    )
        ...
        while True:
            ...
            with columns[0 if col_index < 0 else 1]:
                ...
                if article["image"] == None:
                    if "nulltx" in link:
                        st.image(
                            "https://nulltx.com/wp-content/uploads/2018/10/nulltx-logo-red.png"
                        )
                    if "cryptoslate" in link:
                        st.image(
                            "https://cryptoslate.com/wp-content/themes/cryptoslate-2020/images/cs-media-logo-dark.png"
                        )
                else:
    ...
    if search_option == "Top Headlines":
        news_articles = newscatcherapi.get_search(
            q="*", sources=sources, from_="31/05/2022"
        )
        ...
        if not search_keyword:
            ...
            while True:
                try:
                    news_articles = newscatcherapi.get_search(
                        q=search_keyword, sources=sources, from_="31/05/2022"
                    )
                    ...
                    st.write(
                        "Please enter new search keyword. There is no Crypto News existing with this keyword."
                    )
                    break
